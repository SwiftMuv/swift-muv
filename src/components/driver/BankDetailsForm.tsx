...
export const BankDetailsForm = ({ onSaved, onCancel }: Props) => {
  ...
  const save = async () => {
    ...
    if (!form.account_holder_name || !form.bank_name || !/^\d{5}$/.test(form.transit_number)
      ...
      return toast.error(t("drv.bank.validation"));
    }
    setSaving(true);
    ...
    toast.success(t("drv.bank.linked"));
    onSaved?.();
  };

  return (
    <div className="rounded-xl bg-card border p-4 space-y-3">
      <p className="text-sm font-semibold">{t("drv.bank.title")}</p>
      <div className="space-y-2">
        <Label className="text-xs">{t("drv.bank.holderName")}</Label>
        <Input value={form.account_holder_name} onChange={(e) => setForm({ ...form, account_holder_name: e.target.value })} />
      </div>
      <div className="space-y-2">
        <Label className="text-xs">{t("drv.bank.bankName")}</Label>
        <Input value={form.bank_name} onChange={(e) => setForm({ ...form, bank_name: e.target.value })} />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-2">
          <Label className="text-xs">{t("drv.bank.transit")}</Label>
        ...
        <div className="space-y-2">
          <Label className="text-xs">{t("drv.bank.institution")}</Label>
      ...
      <div className="space-y-2">
        <Label className="text-xs">{t("drv.bank.accountNumber")}</Label>
      ...
      <div className="flex gap-2">
        <Button onClick={save} disabled={saving} className="flex-1 rounded-xl h-11 font-semibold">
          {saving && <Loader2 className="h-4 w-4 animate-spin mr-1" />}{t("drv.bank.save")}
        </Button>
        {onCancel && (
          <Button variant="outline" className="rounded-xl h-11" onClick={onCancel}>{t("common.cancel")}</Button>
        )}
      </div>
    </div>
  );
